...
PowerUserPrivilegesRequired=您必须以管理员或已授权用户身份登录才能安装本程序。
...
ExitSetupTitle=退出安装
ExitSetupMessage=安装尚未完成。如果现在退出，本软件将不被安装。%n%n您可以稍后再运行安装程序来完成安装过程。%n%n是否退出安装？
...
SelectLanguageLabel=选择您喜欢的语言进行安装：
...
WelcomeLabel2=本程序将安装 [name/ver] 到您的计算机中。%n%n强烈建议您在继续安装之前关闭其他所有正在运行的程序，以避免安装过程中可能产生的相互冲突。

; *** "Password" wizard page
WizardPassword=密码
PasswordLabel1=本安装程序受密码保护。
PasswordLabel3=请输入密码，然后单击“下一步”继续。密码是一层保护壳。
...
SelectDirLabel=选择您准备将 [name] 安装到的文件夹，然后单击“下一步”。
...
DirExists=目录：%n%n%1%n%n已存在。 您无论如何都要安装到该目录吗？
...
SelectStartMenuFolderLabel=选择您需要在开始菜单文件夹中创建的程序快捷方式，然后单击“下一步”。
NoIconsCheck=不创建任何图标(&D)
MustEnterGroupName=您必须输入一个文件夹名称。
BadGroupName=文件夹名称不能包含下列的任何一个字符：%n%n%1
NoProgramGroupCheck2=不创建开始菜单文件夹(&D)

; *** "Ready to Install" wizard page
WizardReady=准备安装
ReadyLabel1=安装程序已准备好，即将将 [name] 安装到您的计算机中。
...
InstallingLabel=正在安装 [name] 到您的计算机中，请稍等。
...
ErrorCreatingTemp=正尝试在目标目录中创建文件时出错：
ErrorReadingSource=正尝试读取源文件时出错：
ErrorCopying=正尝试复制文件时出错：
ErrorReplacingExistingFile=正尝试替换已有文件时出错：
ErrorRestartReplace=重新启动替换失败：
ErrorRenamingTemp=正尝试给目标目录中文件重命名时出错：
ErrorRegisterServer=不能注册 DLL/OCX： %1
ErrorRegisterServerMissingExport=DllRegisterServer 输出口没有发现
ErrorRegisterTypeLib=不能注册类库： %1

; *** Post-installation errors
ErrorOpeningReadme=正尝试打开自述文件时出错。
ErrorRestartingComputer=安装程序不能重新启动计算机。请手动进行。

; *** Uninstaller messages
UninstallNotFound=文件 “%1” 不存在。不能卸载。
UninstallOpenError=不能打开文件“%1”。不能卸载。
UninstallUnsupportedVer=卸载记录文件 “%1” 的格式不能被本版本卸载程序识别。不能卸载。
UninstallUnknownEntry=卸载记录文件中遇到不能识别的条目 (%1)
ConfirmUninstall=确认要完全删除 %1 和它所有的部件吗？
OnlyAdminCanUninstall=只能由有管理员权限的用户进行卸载。
UninstallStatusLabel=正在从您的计算机中删除 %1，请等待。
UninstalledAll=%1 已经从您的计算机中成功删除。
UninstalledMost=%1 卸载完成。%n%n某些项目不能移除。您可以手动删除。
UninstalledAndNeedsRestart=要完全卸载 %1， 你必须要重新启动计算机。%n%n是否立即重新启动？
UninstallDataCorrupted=“%1”文件受损。无法卸载
